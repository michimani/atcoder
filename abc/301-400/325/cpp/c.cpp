...
void sensor_traverse(vector<string>& grid, unsigned long i, unsigned long j,
                     vector<string>& visited, int& visited_sensor_count) {
    visited[i][j] = '#';
    visited_sensor_count++;
    for (unsigned long ti = max((unsigned long)(1), i - 1);
         ti <= min(i + 1, (unsigned long)(grid.size() - 1)); ti++) {
        for (unsigned long tj = max((unsigned long)(1), j - 1);
             tj <= min(j + 1, (unsigned long)(grid[0].size())); tj++) {
            if (ti == i && tj == j) {
                continue;
            }
