...
bool traverse(map<int, vector<int>> edges, int prev_vertex, int current_vertex,
              map<int, bool> root, vector<int> root_vec) {
    bool loop = false;
...
int main() {
    ...
    while (v_iter != vertices.end()) {
        bool is_loop = traverse(edges, 0, v_iter->first,
                                {{v_iter->first, true}}, {v_iter->first});
