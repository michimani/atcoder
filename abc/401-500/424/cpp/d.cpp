    ...
    while (t--) {
        ...
        for (int i = 0; i < h; i++) {
            for (int prev_mask = 0; prev_mask < (1 << w); prev_mask++) {
                if (dp[i][prev_mask] == 1e9) continue;
                ...
                for (int cur_mask = 0; cur_mask < (1 << w); cur_mask++) {
                    ...
                    if (valid)
                        dp[i + 1][cur_mask] = min(dp[i + 1][cur_mask],
                                                  dp[i][prev_mask] + changes);
                }
            }
        }

        int ans = 1e9;
        for (int mask = 0; mask < (1 << w); mask++) ans = min(ans, dp[h][mask]);
